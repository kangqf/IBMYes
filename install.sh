...
create_mainfest_file(){
    ...
    cat >  ${SH_PATH}/IBMYes/v2ray-python-cf/manifest.yml  << EOF
    applications:
    - path: .
      name: ${IBM_APP_NAME}
      random-route: true
...
EOF
    ...
    cat >  ${SH_PATH}/IBMYes/v2ray-python-cf/v2ray/config.json  << EOF
    {
        "inbounds": [
            {
                "port": 8080,
...
clone_repo(){
    echo "进行初始化。。。"
	rm -rf IBMYes
    git clone https://github.com/kangqf/IBMYes
    cd IBMYes
    git submodule update --init --recursive
    cd v2ray-python-cf/v2ray
    ...
    chmod 0755 ./*
    cd ${SH_PATH}/IBMYes/v2ray-python-cf
    echo "初始化完成。"
}
